...
@section('content')
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Classe /
                            Formation</th>
                        <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Staff
                            (Principal & Adjoints)</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach ($classes as $classe)
                        <tr class="hover:bg-red-50/20 transition-all group">
                            {{-- Nom Classe & Formation --}}
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="w-12 h-12 bg-gray-900 text-white rounded-2xl flex items-center justify-center font-black italic shadow-lg group-hover:scale-110 transition-transform">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-[#ff002b] font-bold uppercase italic">
                                            {{ $classe->formation->title ?? 'Sans Formation' }}</p>
                                    </div>
                                </div>
                            </td>

                            {{-- Staff Logic: 1 Principal + Remplaçants --}}
                            <td class="px-8 py-6">
                                <div class="space-y-2">
                                    {{-- Formateur Principal --}}
                                    @php
                                        $principal = $classe->formateurs()->wherePivot('is_principal', true)->first();
                                        $adjoints = $classe->formateurs()->wherePivot('is_principal', false)->get();
                                    @endphp

                                    <div class="flex items-center gap-2">
                                        <span
                                            class="w-5 h-5 rounded-md bg-[#ff002b] text-white flex items-center justify-center text-[9px] font-black shadow-sm"
                                            title="Formateur Principal">P</span>
                                        <span
                                            class="text-xs font-bold text-gray-700 italic">{{ $principal->name ?? 'Non assigné' }}</span>
                                    </div>
                            ...
                            {{-- Capacité (Contrainte 25) --}}
                            <td class="px-8 py-6 text-center">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-xl text-[11px] font-black {{ $classe->etudiants_count >= 25 ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600' }}">
                            ...
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button
                                        class="w-9 h-9 flex items-center justify-center rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all"><i
                                            class="fas fa-edit text-xs"></i></button>
                                    <button
                                        class="w-9 h-9 flex items-center justify-center rounded-xl bg-red-50 text-[#ff002b] hover:bg-[#ff002b] hover:text-white transition-all"><i
    ...
    {{-- Modal --}}
    <div id="modal-classe"
        class="hidden fixed inset-0 bg-gray-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-[3rem] shadow-2xl max-w-md w-full p-10 animate-in zoom-in duration-200">
            ...
            <form action="{{ route('classes.store') }}" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label class="text-[10px] font-black text-gray-400 uppercase ml-2 mb-1 block tracking-widest">Nom de la
                        promo</label>
                ...
                <button type="submit"
                    class="w-full bg-[#ff002b] text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg shadow-red-200 hover:bg-red-600 transition-all">Créer
                    la classe</button>
                <button type="button" onclick="toggleModal('modal-classe')"
                    class="w-full text-gray-400 font-bold text-xs uppercase mt-2">Annuler</button>
            </form>
        </div>
    </div>
